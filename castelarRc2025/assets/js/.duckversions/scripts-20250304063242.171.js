...
jQuery(document).ready(function () {
  // Inicializa os tooltips do Bootstrap, se houver elementos com essa funcionalidade
  jQuery('[data-bs-toggle="tooltip"]').tooltip();

  // Exemplo de delegação: se desejar abrir um modal específico ao clicar em uma imagem
  jQuery(document).on('click', '.open-modal-img', function (e) {
    e.preventDefault();
    // Supondo que o elemento possua um data-target com o seletor do modal a abrir
    var targetModal = jQuery(this).data('target');
    var imgUrl = jQuery(this).find('img').data('img-url');
    // Atualiza a imagem do modal, se necessário
    jQuery(targetModal).find('.modal-body img').attr('src', imgUrl);
    // Abre o modal utilizando a API do Bootstrap 5
    var modal = new bootstrap.Modal(document.querySelector(targetModal));
    modal.show();
  });

  // Caso deseje fechar o modal ao clicar fora do conteúdo (opcional, pois o Bootstrap já trata esse caso)
