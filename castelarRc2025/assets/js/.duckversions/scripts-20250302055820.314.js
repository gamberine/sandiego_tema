...
jQuery(document).ready(function () {
  jQuery('a.glink.nturl.notranslate').on('click', function () {
    // Obtém o atributo lang do elemento <html>
    ...
    // Função para "sanitizar" o idioma similar ao sanitize_title_with_dashes do PHP
    var sanitizedLang = currentLang.toLowerCase()
      .replace(/[^a-z0-9]+/g, '-')
      .replace(/(^-|-$)/g, '');

    // Remove classes existentes que comecem com "lang-"
    jQuery('body').removeClass(function (index, className) {
      return (className.match(/\blang-\S+/g) || []).join(' ');
    });

    // Adiciona a nova classe de idioma no body
