...
jQuery(document).ready(function ($) {
  var updateWhatsappLink = function (newLang = null) {
    var currentLang = newLang || $('html').attr('lang');
    var newLink = whatsappLinks['pt-BR']; // Padrão

    // Verifica idiomas específicos
    if (currentLang === 'en') {
  ...
  $(document).on('click', 'a.glink.nturl.notranslate:not(.gt-current-lang)', function () {
    var newLang = $(this).attr('data-gt-lang');
    if (newLang) {
