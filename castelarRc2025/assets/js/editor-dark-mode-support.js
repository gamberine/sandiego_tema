/* global temabasegambIsDarkMode, setTimeout */

// Check the color scheme preference and inject the classes if necessary.
if ( document.body.classList.contains( 'temabasegamb-supports-dark-theme' ) ) {
	temabasegambDarkModeEditorInit();
}
 ...
 * @return {void}
 */
function temabasegambDarkModeEditorInit( attempt ) {
	...
	if ( temabasegambIsDarkMode() ) {
		if ( null === container ) {
			// Try again.
			if ( attempt < maxAttempts ) {
				setTimeout(
					function() {
						temabasegambDarkModeEditorInit( attempt + 1 );
					},
