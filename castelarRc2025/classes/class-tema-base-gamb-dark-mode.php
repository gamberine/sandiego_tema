...
class Tema_Dev_Gamb_Dark_Mode {
	...
	public function __construct() {
		...
		// Add HTML classes.
		add_filter( 'temabasegamb_html_classes', array( $this, 'html_classes' ) );
	...
	public function editor_custom_color_variables() {
		...
		if ( $should_respect_color_scheme && Tema_Dev_Gamb_Custom_Colors::get_relative_luminance_from_hex( $background_color ) > 127 ) {
			// Add Dark Mode variable overrides.
			wp_add_inline_style(
				'tema-base-gamb-custom-color-overrides',
				'.is-dark-theme.is-dark-theme .editor-styles-wrapper { --global--color-background: var(--global--color-dark-gray); --global--color-primary: var(--global--color-light-gray); --global--color-secondary: var(--global--color-light-gray); --button--color-text: var(--global--color-background); --button--color-text-hover: var(--global--color-secondary); --button--color-text-active: var(--global--color-secondary); --button--color-background: var(--global--color-secondary); --button--color-background-active: var(--global--color-background); --global--color-border: #9ea1a7; --table--stripes-border-color: rgba(240, 240, 240, 0.15); --table--stripes-background-color: rgba(240, 240, 240, 0.15); }'
			);
		}
		wp_enqueue_script(
			'temabasegamb-dark-mode-support-toggle',
		...
		wp_enqueue_script(
			'temabasegamb-editor-dark-mode-support',
			get_template_directory_uri() . '/assets/js/editor-dark-mode-support.js',
			array( 'temabasegamb-dark-mode-support-toggle' ),
			'1.0.0',
			true
		);
	}
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_style( 'tt1-dark-mode', $url, array( 'tema-base-gamb-style' ), wp_get_theme()->get( 'Version' ) ); 
	}
	...
	public function customize_controls_enqueue_scripts() {
		...
		wp_enqueue_script(
			'temabasegamb-customize-controls',
			get_template_directory_uri() . '/assets/js/customize.js',
			array( 'customize-base', 'customize-controls', 'underscore', 'jquery', 'temabasegamb-customize-helpers' ),
			'1.0.0',
			true
		);
	}
	...
	public function customizer_controls( $wp_customize ) {
		...
		if ( is_object( $colors_section ) ) {
			$colors_section->title = __( 'Colors & Dark Mode', 'temabasegamb' );
		}
		...
		$wp_customize->add_control(
			new Tema_Dev_Gamb_Customize_Notice_Control(
				$wp_customize,
				'respect_user_color_preference_notice',
				array(
					'section'         => 'colors',
					...
					'active_callback' => function() {
						return 127 >= Tema_Dev_Gamb_Custom_Colors::get_relative_luminance_from_hex( get_theme_mod( 'background_color', 'D1E4DD' ) );
					},
				)
			)
		);
		...
		$description  = '<p>';
		$description .= sprintf(
			/* translators: %s: Tema Dev-Gamb support article URL. */
			__( 'Dark Mode is a device setting. If a visitor to your site requests it, your site will be shown with a dark background and light text. <a href="%s">Learn more about Dark Mode.</a>', 'temabasegamb' ),
			esc_url( __( 'https://wordpress.org/support/article/tema-base-gamb/#dark-mode-support', 'temabasegamb' ) )
		);
		$description .= '</p>';
		$description .= '<p>' . __( 'Dark Mode can also be turned on and off with a button that you can find in the bottom right corner of the page.', 'temabasegamb' ) . '</p>';
		...
		$wp_customize->add_control(
			...
			array(
				'type'            => 'checkbox',
				'section'         => 'colors',
				'label'           => esc_html__( 'Dark Mode support', 'temabasegamb' ),
				'priority'        => 110,
	...
	public function html_classes( $classes ) {
		...
		$background_color            = get_theme_mod( 'background_color', 'D1E4DD' );
		$should_respect_color_scheme = get_theme_mod( 'respect_user_color_preference', false );
		if ( $should_respect_color_scheme && 127 <= Tema_Dev_Gamb_Custom_Colors::get_relative_luminance_from_hex( $background_color ) ) {
	...
	public function admin_body_classes( $classes ) {
		...
		if ( $current_screen->is_block_editor() ) {
			...
			if ( $should_respect_color_scheme && Tema_Dev_Gamb_Custom_Colors::get_relative_luminance_from_hex( $background_color ) > 127 ) {
				$classes .= ' temabasegamb-supports-dark-theme';
			}
		}
	...
	public function switch_should_render() {
		...
		return (
			...
			! $is_IE &&
			127 <= Tema_Dev_Gamb_Custom_Colors::get_relative_luminance_from_hex( get_theme_mod( 'background_color', 'D1E4DD' ) )
		);
	}
	...
	public function the_html( $attrs = array() ) {
		...
		printf(
			...
			esc_html__( 'Dark Mode: %s', 'temabasegamb' ),
		...
		<style>
			...
			#dark-mode-toggler > span::before {
				content: '<?php esc_attr_e( 'Off', 'temabasegamb' ); ?>';
			}
			#dark-mode-toggler[aria-pressed="true"] > span::before {
				content: '<?php esc_attr_e( 'On', 'temabasegamb' ); ?>';
			}
	...
	public function add_privacy_policy_content() {
		...
		$content = '<p class="privacy-policy-tutorial">' . __( 'Tema Dev-Gamb uses LocalStorage when Dark Mode support is enabled.', 'temabasegamb' ) . '</p>'
				. '<strong class="privacy-policy-tutorial">' . __( 'Suggested text:', 'temabasegamb' ) . '</strong> '
				. __( 'This website uses LocalStorage to save the setting when Dark Mode support is turned on or off.<br> LocalStorage is necessary for the setting to work and is only used when a user clicks on the Dark Mode button.<br> No data is saved in the database or transferred.', 'temabasegamb' );
		wp_add_privacy_policy_content( 'Tema Dev-Gamb', wp_kses_post( wpautop( $content, false ) ) );
	}
