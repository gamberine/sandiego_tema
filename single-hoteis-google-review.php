...
<section class="section-pad bg-light">
  <div class="container">
    <?php
           = sd_field('hotel_google_place_id');
     = sd_get_google_reviews(, 8);
    ?>
    <h2 class="sd-title h3 mb-4">Depoimentos dos nossos H&oacute;spedes</h2>
        ...
        <?php foreach (  as  ) : ?>
          <article class="col">
            <div class="sd-card h-100 p-3 d-flex flex-column gap-2">
              <div class="fw-semibold"><?php echo esc_html( ['author'] ); ?></div>
              <?php if ( ! empty( ['time'] ) ) : ?>
                <div class="small text-muted"><?php echo esc_html( ['time'] ); ?></div>
              <?php endif; ?>
              <div class="sd-google-stars" aria-label="<?php echo esc_attr( ['rating'] ); ?> estrelas">
                <?php
                 = (int) floor( ['rating'] );
                for (  = 1;  <= 5; ++ ) {
              ...
              <?php if ( ! empty( ['text'] ) ) : ?>
                <p class="mb-0 small"><?php echo esc_html( ['text'] ); ?></p>
              <?php endif; ?>
    ...
    <?php else : ?>
      <div class="sd-carousel-depo">
        <?php
         = new WP_Query( [ 'post_type' => 'depoimento', 'posts_per_page' => 6 ] );
        while ( ->have_posts() ) {
          ->the_post();
