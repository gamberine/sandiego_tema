...
//     var newLink = link_whatsapp_pt; // valor padrão: pt

//     if (currentLang === 'en') {
//       newLink = link_whatsapp_en;
//     } else if (currentLang === 'ca') {
//       newLink = link_whatsapp_ca;
//     }
...
jQuery(document).ready(function ($) {
  /**
   * Atualiza o link do botão de WhatsApp de acordo com o idioma atual (definido no <html lang="...">).
   */
  var updateWhatsappLink = function () {
    // Lê o atributo "lang" do elemento <html>
    var currentLang = $('html').attr('lang');
    // Define como padrão o link em pt
    var newLink = whatsapp_links.pt;

    // Se for "en" ou "ca", ajusta o link correspondente
    if (currentLang === 'en') {
      newLink = whatsapp_links.en;
    } else if (currentLang === 'ca') {
      newLink = whatsapp_links.ca;
    }

    // Aplica o link no elemento .btnWhatsAppFooter
    $('.btnWhatsAppFooter').attr('href', newLink);
  };

  // Chama a função assim que a página carrega
  updateWhatsappLink();

  // Se o usuário alterar o idioma via plugin GTranslate, aguarda um pequeno delay e atualiza o link
  $('.gtranslate_wrapper a').on('click', function () {
    setTimeout(function () {
      updateWhatsappLink();
    }, 500);
  });
...
// Fim do script para atualizar o link do WhatsApp com base no idioma do site
