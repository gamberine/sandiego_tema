        ...
        ?>
                ...
                <?php if ($selecao && $imagem_banner): ?>
                    ...
                    <div class="slide-items banner"
                        style="background: url('<?php echo $background_desktop; ?>');"
                        data-mobile-bg="<?php echo $background_mobile; ?>">
                    <?php elseif (!$selecao && $video_banner): ?>
                        <!-- Exibe vídeo se select_imagem for false -->
                        <div class="slide-items video" style="background: var(--gradientBanner) !important; background-size: cover; background-blend-mode: multiply;">
                            <video autoplay muted loop>
                                <source src="<?php echo esc_url($video_banner); ?>" type="video/mp4">
                            </video>
                        <?php else: ?>
                            <!-- Exibe um slide padrão caso nenhum campo esteja configurado -->
                            <div class="slide-items" style="background: var(--gradientBanner); background-size: cover;">
                                <p>Slide não configurado corretamente.</p>
                            <?php endif; ?>
                            <div class="formaBanner">
                                <!-- <h1>< ?php the_title(); ?></h1> -->
                                <div class="animationFade vibrate"><?php echo wp_kses_post(get_field('texto_banner')); ?></div>
