...
// scripts.js - Código em JavaScript puro (sem uso de $) para ativar o modal em qualquer .gallery
document.addEventListener('click', function (event) {
  // Verifica se o clique ocorreu em um link (<a>) dentro de um elemento com classe .gallery
  var galleryLink = event.target.closest('.gallery a');
  if (!galleryLink) return; // Se não for, não faz nada

  event.preventDefault(); // Impede o comportamento padrão do link

  // Obtém o contêiner da galeria em que o link foi clicado
  var galleryContainer = galleryLink.closest('.gallery');
  if (!galleryContainer) return;

  // Seleciona todos os links (<a>) presentes na galeria
  var galleryLinks = galleryContainer.querySelectorAll('a');
  var items = [];
  galleryLinks.forEach(function (link) {
    var href = link.getAttribute('href');
    // Procura o elemento <figcaption> dentro do item da galeria
    var figure = link.closest('.gallery-item');
    var caption = '';
    if (figure) {
  ...
  // Determina o índice do link clicado dentro da lista da galeria
  var galleryLinksArray = Array.prototype.slice.call(galleryLinks);
  var activeIndex = galleryLinksArray.indexOf(galleryLink);

  // Monta os slides do carousel dinamicamente
  var carouselInnerHtml = '';
  items.forEach(function (item, index) {
    carouselInnerHtml += '<div class="carousel-item' + (index === activeIndex ? ' active' : '') + '">';
    carouselInnerHtml += '<img src="' + item.href + '" class="d-block w-100" alt="">';
    carouselInnerHtml += '</div>';
  });

  // Seleciona o modal dinâmico (HTML inserido no footer)
  var modal = document.getElementById('dynamicGalleryModal');
  if (!modal) {
    console.error('Modal container com o ID "dynamicGalleryModal" não foi encontrado no footer.');
    return;
  }

  // Atualiza o conteúdo do carousel dentro do modal
  var carouselInner = modal.querySelector('.carousel-inner');
  if (carouselInner) {
    carouselInner.innerHTML = carouselInnerHtml;
  }

  // Atualiza a área de legenda abaixo do carousel
  var captionEl = modal.querySelector('#carouselCaption');
  if (captionEl) {
    captionEl.innerHTML = items[activeIndex].caption;
  }

  // Inicializa (ou reinicializa) o carousel do Bootstrap
  var carouselElement = modal.querySelector('.carousel');
  if (carouselElement) {
    // Caso já exista uma instância, a descarte para evitar conflitos
    var existingCarousel = bootstrap.Carousel.getInstance(carouselElement);
    if (existingCarousel) {
      existingCarousel.dispose();
    }
    // Cria uma nova instância sem autoplay
    var carouselInstance = new bootstrap.Carousel(carouselElement, {
      interval: false
    });

    // Atualiza a legenda sempre que o slide mudar
    carouselElement.addEventListener('slid.bs.carousel', function (e) {
      var newIndex = e.to;
      if (captionEl) {
        captionEl.innerHTML = items[newIndex].caption;
      }
    });
  }
