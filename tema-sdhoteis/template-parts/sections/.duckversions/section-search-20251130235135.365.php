...
// ======================================================
// 1. CARREGAR AS UNIDADES DIRETAMENTE DA API OMNIBEES
// ======================================================

window.hotelsLive = [];

fetch("https://book.omnibees.com/api/hotel/GetHotels?c=9627&currencyId=16&culture=pt-BR")
  .then(r => r.json())
  .then(data => {
    window.hotelsLive = (data.Hotels || []).sort((a, b) => {
      return (a.PropertySequence ?? 999) - (b.PropertySequence ?? 999);
    });
  })
  .catch(err => {
    console.error("Erro ao carregar hotéis da Omnibees:", err);
  });

function getHotels() {
  return window.hotelsLive || [];
}


// ======================================================
// 2. AUTOCOMPLETE (TOTALMENTE COMPATÍVEL COM SEU HTML)
// ======================================================
...
function renderHotels(term) {
  ...
  if (termo !== "") {
    list = hotels.filter(h =>
      h.Property_Name.toLowerCase().includes(termo)
    );
  }
...
// ======================================================
// 3. CLICK EM TODA A ÁREA DO BLOCO (SEU LAYOUT MANTIDO)
// ======================================================

function sdFocarInput(wrapper) {
...
// ======================================================
// 6. BUSCAR (URL FINAL DO OMNIBEES)
// ======================================================
